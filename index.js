...
for (const feature of geojson.features) {
    ...
    // Check if the feature represents the current location
    if (feature.properties.title === 'Your Location') {
        // Create HTML element for the current location marker
        const currentLocationEl = document.createElement('div');
        currentLocationEl.className = 'marker-current-location';

        // Make a marker for the current location and add it to the map
        ...
        // Make a marker for other features and add them to the map
        new mapboxgl.Marker(el)
            .setLngLat(feature.geometry.coordinates)
            .setPopup(
                new mapboxgl.Popup({ offset: 25 }) // Add popups
